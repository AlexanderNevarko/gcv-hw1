...
def get_view(
    ...
    intrinsics_i = intrinsics_dict[i]
    ...
    pose_i = CameraPose(extrinsics_i)
    imaging_i = RaycastingImaging(resolution_image=image_i.shape,
                                  resolution_3d=intrinsics_i['resolution_3d'])
...
def pairwise_interpolate_predictions(
    ...
    distances_to_nearest = np.sqrt(np.power(distances_2d, 2) + 
                                   np.power(reprojected_j[:, 2][:, None] - 
                                            image_i.flatten()[nn_indexes_in_i], 2)).min(axis=-1)
    interp_mask = distances_to_nearest < distance_interpolation_threshold
    ...
    points_from_j_nns = np.concatenate([uv_i[nn_indexes_in_i], image_i.flatten()[nn_indexes_in_i][:, :, None]], axis=-1)
    distances_i_for_cycle = distances_i.flatten()[nn_indexes_in_i]
    for idx in tqdm(np.argwhere(interp_mask)):
        idx = idx[0]
        try:
            ...
            interpolator = interpolate.interp2d(x=points_from_j_nns[idx, :, 0],
                                                y=points_from_j_nns[idx, :, 1],
                                                z=distances_i_for_cycle[idx])
            distances_j_interp[idx] = interpolator(*reprojected_j[idx, :2])
